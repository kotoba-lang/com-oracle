(ns oracle.main
  "Kotodama WASM entrypoint for the Oracle clean-room actor (L4) — Clojure port.
  ...
  facts (`oracle.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "oracle")
(def tier "L4")
...
(def entity-specs
  [{:entity "GLAccount"     :plural "glaccounts"     :id-prefix "oracle_gla"
    :fields [:code :name :type :currency :balance]  :required [:code :name]
    :coerce {:balance :float}                        :refs {}}
   {:entity "JournalEntry"  :plural "journalentries" :id-prefix "oracle_jou"
    :fields [:accountId :debit :credit :memo :postedAt] :required [:debit :credit]
    :coerce {:debit :float :credit :float}           :refs {}}
   {:entity "Invoice"       :plural "invoices"       :id-prefix "oracle_inv"
    :fields [:customerId :number :total :currency :status] :required [:number :total]
    :coerce {:total :float}                          :refs {}}
   {:entity "PurchaseOrder" :plural "purchaseorders" :id-prefix "oracle_pur"
    :fields [:vendorId :number :total :status]      :required [:number :total]
    :coerce {:total :float}                          :refs {:vendorId "Vendor"}}
   {:entity "Vendor"        :plural "vendors"        :id-prefix "oracle_ven"
    :fields [:name :taxId :terms :currency]         :required [:name :terms]
    :coerce {}                                       :refs {}}
   {:entity "CostCenter"    :plural "costcenters"    :id-prefix "oracle_cos"
    :fields [:code :name :ownerId]                  :required [:code :name]
    :coerce {}                                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"oracle.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "oracle-compat" :tier tier :entities entities} 200])
...
(defn start! [] :oracle-compat/ready)
